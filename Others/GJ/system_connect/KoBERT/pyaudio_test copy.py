...
class VITOOpenAPIClient:
    def __init__(self, client_id, client_secret):
        ...
        self.speaker_separation_model = joblib.load('speaker_separation_model.pkl')
    ...
    async def streaming_transcribe(self, audio_generator, config=None):
        ...
        async def make_amplitude_list(data):
            ...
            #print(y)
            self.amplitude_list = np.append(self.amplitude_list, new_buffer)
        ...
        def analyzer(msg):
            ...
            #print(edited_data)
            finite_mask = np.isfinite(edited_data)
            finite_values = edited_data[finite_mask]
            new_mfccs = librosa.feature.mfcc(finite_values, sr=44100, n_mfcc=13)
            
            if new_mfccs.shape[1] < 431:
                ref_labels2 = np.full((13,431 - new_mfccs.shape[1]), 0)
                new_mfccs = np.concatenate((new_mfccs, ref_labels2), axis=1)
            elif new_mfccs.shape[1] > 431:
                new_mfccs = new_mfccs[:,:431]
            result = self.speaker_separation_model.predict(new_mfccs.T)
            #print(result)

...
# 초당 16000개의 frame을 3200 frame 단위로 나눠서 전달
CHUNK = 3200
...
class VITOOpenAPIClient:
    ...
    async def streaming_transcribe(self, audio_generator, config=None):
        ...
        async def analyzer(msg):
            edited_data = await split_wav(16000, msg["start_at"], msg["start_at"]+msg["duration"])
        ...
        async def split_wav(sample_rate, start, end):
            start *= sample_rate
            end *= sample_rate
...
if __name__ == "__main__":
    ...
    audio_generator = iter(lambda: stream.read(CHUNK), b"")
